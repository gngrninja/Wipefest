...
export module SistersOfTheMoonInsights {
    ...
    export function TwilightGlaive(events: FightEvent[]): Insight {
        ...
        let insight = `You unnecessarily intercepted ${MarkupHelper.Ability(damageEvents[0].ability)} ${MarkupHelper.Info(totalHits)} times.`;
    ...
    export function AstralVulnerability(events: FightEvent[]): Insight {
        ...
        let insight = `You gained ${MarkupHelper.Info(6)} stacks of ${MarkupHelper.Ability(debuffEvents[0].ability)} ${MarkupHelper.Info(totalFrequency)} times.`;
    ...
    export function MoonBurn(events: FightEvent[]): Insight {
        ...
        let insight = `You had a total ${MarkupHelper.Ability(debuffEvents[0].ability)} duration of ${MarkupHelper.Info(Timestamp.ToSeconds(totalDuration))}, with ${MarkupHelper.Info(playersAndDurationsOverThreshold.length)} lasting longer than ${Timestamp.ToSeconds(threshold)}.`;
