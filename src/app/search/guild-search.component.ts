...
export class GuildSearchComponent implements OnInit {
  ...
  get favouriteGuild(): string {
    return this.localStorage.get(this.guildKey);
  }
  set favouriteGuild(value: string) {
    this.localStorage.setOrRemove(this.guildKey, value);
  }
  ...
  searchByGuild() {
    this.router
      .navigate([
        `/guild/${this.guild}/${this.clean(this.realm)}/${this.clean(
          this.region
        )}`
      ])
      .then(success => {
        if (success) {
          this.logger.logGuildSearch(this.guild, this.realm, this.region);
        }
      });
  }
