...
export class WarcraftLogsService {
  ...
  getParses(
    ...
    const batch = partitions.map(x =>
      this.getParsesForPartition(
        character,
        realm,
        region,
        zone,
  ...
  private getParsesForPartition(
    ...
    const damageParses = this.getParsesForMetric(
      character,
      realm,
      region,
      zone,
    ...
    const healingParses = this.getParsesForMetric(
      character,
      realm,
      region,
      zone,
    ...
    const batch = [damageParses, healingParses];

    return Observable.forkJoin(batch).map(x => [].concat.apply([], x)); // Flatten arrays into one array
  }

  private getParsesForMetric(
    character: string,
    realm: string,
    region: string,
    zone: number,
    ...
    return this.get(
      this.url +
        ...
        zone +
        '&metric=' +
        (metric == Metric.Damage ? 'dps' : 'hps') +
        '&partition=' +
        partition
    )
      .catch(error => this.handleError(error))
      .map(response =>
        (response.json() as Parse[]).map(x => {
          const difficulty = x;
          difficulty.specs = difficulty.specs.filter(y => {
            const spec = this.classesService.getSpecializationByName(
              y.class,
              y.spec
            );
            if (!spec) {
            ...
            if (metric == Metric.Damage && spec.role != 'Healer') {
              return true;
            }
            if (metric == Metric.Healing && spec.role == 'Healer') {
              return true;
            }
            return false;
          });
