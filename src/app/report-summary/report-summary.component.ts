...
export class ReportSummaryComponent implements OnInit {
  ...
  private handleRoute(params: Params) {
    ...
    this.loading = true;
    this.warcraftLogsService.getReport(this.reportId).subscribe(
      report => {
        this.error = null;
      ...
      if (isABoss) {
        if (fight.difficulty == 5) {
          const alreadyBeenMapped =
            this.mythicEncounters.filter(x => x.some(y => y.boss == fight.boss))
              .length > 0;
        ...
        if (fight.difficulty == 4) {
          const alreadyBeenMapped =
            this.heroicEncounters.filter(x => x.some(y => y.boss == fight.boss))
              .length > 0;
          if (!alreadyBeenMapped) {
        ...
        if (fight.difficulty == 3) {
          const alreadyBeenMapped =
            this.normalEncounters.filter(x => x.some(y => y.boss == fight.boss))
              .length > 0;
          if (!alreadyBeenMapped) {
