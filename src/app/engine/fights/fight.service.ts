...
export class FightService {
    ...
    getFight(report: Report, fightInfo: FightInfo, eventConfigs: EventConfig[]): Observable<Fight> {
        ...
        const combatEvents$ = this.combatEventService.getCombatEvents(report, fightInfo, eventConfigs);
        ...
        return Observable.forkJoin([deaths$, combatEvents$]).map(results => {
            ...
            const events = this.fightEventService.getEvents(report, fightInfo, eventConfigs, combatEvents, deaths);
            const raid = RaidFactory.Get(combatEvents.filter(x => x.type === "combatantinfo"), report.friendlies, this.specializationsService);
            const insights = this.insightService.getInsights(fightInfo.boss, new InsightContext(report, fightInfo, raid, events));
