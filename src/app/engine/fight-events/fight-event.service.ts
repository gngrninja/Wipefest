...
import { HeroismEvent } from 'app/fight-events/models/heroism-event';
import { AbilityEvent, Ability } from 'app/fight-events/models/ability-event';
...
export class FightEventService {
    ...
    getEvents(report: Report, fight: FightInfo, configs: EventConfig[], combatEvents: CombatEvent[], deaths: Death[]): FightEvent[] {
        let events: FightEvent[] = [].concat.apply([], configs.map(config => {
            let matchingCombatEvents = this.filterToMatchingCombatEvents(config, combatEvents, fight, report);
    ...
    private getHeroismEvents(report: Report, fight: FightInfo, config: EventConfig, combatEvents: CombatEvent[]): HeroismEvent[] {
    ...
    private getAbilityEvents(report: Report, fight: FightInfo, config: EventConfig, combatEvents: CombatEvent[]): AbilityEvent[] {
        let events = combatEvents.map(
            ...
            .filter(x => !(!config.includePetTargets && x.target && x.target.hasOwnProperty("petOwner")));

        return events;
    }

    private getDamageEvents(report: Report, fight: FightInfo, config: EventConfig, combatEvents: CombatEvent[], timestampOffset: number, isChild: boolean): DamageEvent[] {
        let events = combatEvents.map(
            x => new DamageEvent(
                config,
                ...
                x.overkill,
                isChild))
            .filter(x => x.target && !(!config.includePetTargets && x.target.hasOwnProperty("petOwner")));;

        return events;
    }

    private getPhaseChangeEvents(report: Report, fight: FightInfo, config: EventConfig, combatEvents: CombatEvent[]): PhaseChangeEvent[] {
    ...
    private getDebuffEvents(report: Report, fight: FightInfo, config: EventConfig, combatEvents: CombatEvent[]): DebuffEvent[] {
        let events = combatEvents.map(
            x => new DebuffEvent(
                config,
                x.timestamp - fight.start_time,
    ...
    private getDebuffStackEvents(report: Report, fight: FightInfo, config: EventConfig, combatEvents: CombatEvent[]): DebuffStackEvent[] {
        let events = combatEvents.map(
    ...
    private getRemoveDebuffEvents(report: Report, fight: FightInfo, config: EventConfig, combatEvents: CombatEvent[]): RemoveDebuffEvent[] {
        let events = combatEvents.map(
    ...
    private getInterruptEvents(report: Report, fight: FightInfo, config: EventConfig, combatEvents: CombatEvent[]): InterruptEvent[] {
        let events = combatEvents.map(
    ...
    private getSpawnEvents(report: Report, fight: FightInfo, config: EventConfig, combatEvents: CombatEvent[]): SpawnEvent[] {
        let events = combatEvents.map(
    ...
    private getDeathEvents(report: Report, fight: FightInfo, config: EventConfig, deaths: Death[]): DeathEvent[] {
        let events = deaths.map((death, index) =>
            new DeathEvent(
                ...
                death.damage.total,
                death.healing.total,
                this.getEventsForConfig(report, fight, new EventConfig({ eventType: "damage" }), death.events, deaths, fight.start_time - death.timestamp, true)));

        return events;
    }

    private getTitleEvents(report: Report, fight: FightInfo, config: EventConfig, combatEvents: CombatEvent[]): TitleEvent[] {
